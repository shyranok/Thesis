        ...
        with torch.no_grad():
            for idx_scale, scale in enumerate(self.scale):
                ...
                for idx_img, (hr, filename, _) in enumerate(self.loader_test):
                    ...
                    torch.cuda.empty_cache() #empty cache to create more memory space for testing
                    print(idx_img) #print every tested image

                    

                self.ckp.log[-1, idx_scale] = eval_psnr / len(self.loader_test)
                self.ckp.write_log(
                    '[Epoch {}---{} x{}]\tPSNR: {:.3f} SSIM: {:.4f}\t(raw: {}, {}, {})'.format(
                        self.args.resume,
                        self.args.data_test,
                        scale,
                        eval_psnr / len(self.loader_test),
                        eval_ssim / len(self.loader_test),
                        eval_psnr, #display raw values PSNR
                        eval_ssim, #display raw values SSIM
